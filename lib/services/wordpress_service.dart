...
class WordpressService {
  ...
  Future<List<Article>> fetchCategoryArticles(String url) async {
    List<Article> articles = new List();

    // String url =
    //     'http://lesoirdebko.ml/wp-json//wp/v2/posts?_embed&categories=${categoryID}&_fields=id,modified_gmt,excerpt,title,content,_links';
    print('fetching articles...');
    http.Response response = await http.get(url);
    //check if request is OK
    if (response.statusCode == 200) {
